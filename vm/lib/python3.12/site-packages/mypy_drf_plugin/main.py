...
class NewSemanalDRFPlugin(Plugin):
    def _get_currently_defined_serializers(self) -> Dict[str, int]:
        base_serializer_sym = self.lookup_fully_qualified(
            fullnames.BASE_SERIALIZER_FULLNAME
        )
        if base_serializer_sym is not None and isinstance(
            base_serializer_sym.node, TypeInfo
        ):
            serializer_bases: Dict[str, int] = (
                base_serializer_sym.node.metadata.setdefault("drf", {}).setdefault(
