...
def get_django_metadata_bases(
    model_info: TypeInfo, key: Literal["baseform_bases", "queryset_bases"]
) -> Dict[str, int]:
...
def get_reverse_manager_info(
    ...
    manager_fullname = (
        get_django_metadata(model_info).get("reverse_managers", {}).get(derived_from)
    )
...
def set_reverse_manager_info(
    model_info: TypeInfo, derived_from: str, fullname: str
) -> None:
    get_django_metadata(model_info).setdefault("reverse_managers", {})[
        derived_from
    ] = fullname
...
def set_many_to_many_manager_info(
    to: TypeInfo, derived_from: str, manager_info: TypeInfo
) -> None:
    get_django_metadata(to).setdefault("m2m_managers", {})[
        derived_from
    ] = manager_info.fullname
...
def lookup_class_typeinfo(
    api: TypeChecker, klass: Optional[type]
) -> Optional[TypeInfo]:
    if klass is None:
        return None
...
def is_optional(typ: MypyType) -> bool:
    return isinstance(typ, UnionType) and any(
        isinstance(item, NoneTyp) for item in typ.items
    )
...
def make_typeddict(
    ...
    typed_dict_type = TypedDictType(
        fields, required_keys=required_keys, fallback=fallback_type
    )
...
def is_abstract_model(model: TypeInfo) -> bool:
    ...
    if (
        meta is not None
        and isinstance(meta.node, TypeInfo)
        and "abstract" in meta.node.names
    ):
...
def is_model_type(info: TypeInfo) -> bool:
    return info.metaclass_type is not None and info.metaclass_type.type.has_base(
        fullnames.MODEL_METACLASS_FULLNAME
    )
...
def get_model_from_expression(
    ...
    *,
    ...
    if lazy_reference is not None:
        model_info = resolve_lazy_reference(
            lazy_reference, api=api, django_context=django_context, ctx=expr
        )
        if model_info is not None:
