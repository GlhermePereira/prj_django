...
class NewSemanalDjangoPlugin(Plugin):
        ...
        if (
            ...
            return [
                self._new_dependency(
                    self.django_context.django_settings_module, PRI_MED
                )
            ]
        ...
        if self.django_context.settings:
            if file.fullname == "django.contrib.auth" or file.fullname in {
                ...
                try:
                    ...
                    return []
                return [
                    self._new_dependency(auth_user_module),
                    self._new_dependency("django_stubs_ext"),
                ]
    ...
    @cached_property
    def manager_and_queryset_method_hooks(
        self,
    ) -> Dict[str, Callable[[MethodContext], MypyType]]:
        typecheck_filtering_method = partial(
            orm_lookups.typecheck_queryset_filter, django_context=self.django_context
        )
        return {
            "values": partial(
    ...
    def get_method_hook(
        ...
        if method_name in self.manager_and_queryset_method_hooks:
            ...
            if info and helpers.has_any_of_bases(
                info,
                [fullnames.QUERYSET_CLASS_FULLNAME, fullnames.MANAGER_CLASS_FULLNAME],
            ):
    ...
    def get_base_class_hook(
        ...
        if (
            sym is not None
            and isinstance(sym.node, TypeInfo)
            and helpers.is_model_type(sym.node)
        ):
        ...
        if (
            sym is not None
            and isinstance(sym.node, TypeInfo)
            and sym.node.has_base(fullnames.QUERYSET_CLASS_FULLNAME)
        ):
