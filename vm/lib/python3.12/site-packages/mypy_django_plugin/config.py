...
class DjangoPluginConfig:
    ...
    def parse_toml_file(self, filepath: Path) -> None:
        ...
        django_settings_module = config.get("django_settings_module") or os.getenv(
            DJANGO_SETTINGS_ENV_VAR
        )
    ...
    def parse_ini_file(self, filepath: Path) -> None:
        ...
        if parser.has_option(section, "django_settings_module"):
            django_settings_module = parser.get(
        ...
        try:
            self.strict_settings = parser.getboolean(
                section, "strict_settings", fallback=True
            )
        except ValueError:
