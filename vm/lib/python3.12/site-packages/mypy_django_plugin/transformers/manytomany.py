...
def fill_model_args_for_many_to_many_field(
    *,
    ...
    args = get_m2m_arguments(
        ctx=ctx, model_info=model_info, django_context=django_context
    )
    if args is None:
        return default_return_type
        ...
        if m2m_throughs:
            ...
            if field_name is not None:
                ...
                if through_model_fullname is not None:
                    through_model_info = helpers.lookup_fully_qualified_typeinfo(
                        checker, through_model_fullname
                    )
                    if through_model_info is not None:
                        through = M2MThrough(
                            arg=through_arg, model=Instance(through_model_info, [])
                        )

    return M2MArguments(to=to, through=through)


def get_related_manager_and_model(
    ctx: MethodContext,
) -> Optional[Tuple[Instance, Instance, Instance]]:
    """
