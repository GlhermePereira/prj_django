...
def get_type_of_settings_attribute(
    ...
    plugin_config: DjangoPluginConfig,
) -> MypyType:
        ...
        if module is not None:
            ...
            if sym is not None:
                if sym.type is None:
                    ...
                    typechecker_api.handle_cannot_determine_type(
                        setting_name, ctx.context
                    )
                    return ctx.default_attr_type
                return sym.type
    ...
    if not plugin_config.strict_settings and hasattr(
        django_context.settings, setting_name
    ):
