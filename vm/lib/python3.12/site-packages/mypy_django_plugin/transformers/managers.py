...
def get_method_type_from_dynamic_manager(
    ...
    is_fallback_queryset = queryset_info.metadata.get("django", {}).get(
        "any_fallback_queryset", False
    )
...
def _process_dynamic_method(
    ...
    *,
    ...
    queryset_instance = Instance(
        queryset_info, (manager_model_type,) * len(queryset_info.type_vars)
    )
    ...
    if _has_compatible_type_vars(base_that_has_method):
        ...
        if (
            ...
            ret_type = _replace_type_var(
                ret_type, base_that_has_method.defn.type_vars[0].fullname, typed_var[0]
            )
            args_types = [
                _replace_type_var(
                    arg_type,
                    base_that_has_method.defn.type_vars[0].fullname,
                    typed_var[0],
                )
    ...
    if base_that_has_method.self_type:
        ...
        ret_type = _replace_type_var(
            ret_type, base_that_has_method.self_type.fullname, queryset_instance
        )
...
def _replace_type_var(
    ret_type: MypyType, to_replace: str, replace_by: MypyType
) -> MypyType:
    """
    ...
    if isinstance(ret_type, TypeVarType) and ret_type.fullname == to_replace:
        ...
        ret_type.args = tuple(
            _replace_type_var(item, to_replace, replace_by) for item in ret_type.args
        )
    ...
    if isinstance(ret_type, ProperType) and hasattr(ret_type, "items"):
        ...
        ret_type.items = [
            _replace_type_var(item, to_replace, replace_by) for item in ret_type.items
        ]
    return ret_type
...
def create_new_manager_class_from_from_queryset_method(
    ...
    new_manager_info = create_manager_info_from_from_queryset_call(
        semanal_api, ctx.call, ctx.name
    )
...
def register_dynamically_created_manager(
...
def create_manager_info_from_from_queryset_call(
    ...
    if (
        ...
        or not call_expr.callee.expr.node.has_base(
    ...
    base_manager_info, queryset_info = (
        call_expr.callee.expr.node,
        call_expr.args[0].node,
    )
    ...
    if (
        ...
        and manager_sym.node.metadata.get("django", {}).get("from_queryset_manager")
        == queryset_info.fullname
    ):
    ...
    if name is not None and name != new_manager_info.name:
        ...
        module.names[name] = SymbolTableNode(
            GDEF, new_manager_info, plugin_generated=True
        )

    module.names[new_manager_info.name] = SymbolTableNode(
        GDEF, new_manager_info, plugin_generated=True
    )
...
def create_manager_class(
    ...
    if with_unique_name:
        ...
        manager_info = helpers.create_type_info(
            name, api.cur_mod_id, bases=[base_manager_instance]
        )
...
def populate_manager_from_queryset(
...
def add_as_manager_to_queryset_class(ctx: ClassDefContext) -> None:
    ...
    manager_sym = semanal_api.lookup_fully_qualified_or_none(
    ...
    if (
        ...
        and existing_sym.node.metadata.get("django", {}).get("from_queryset_manager")
        == queryset_info.fullname
    ):
        # Reuse an identical, already generated, manager
    ...
    add_method_to_class(
        ...
        return_type=Instance(
            new_manager_info, [AnyType(TypeOfAny.from_omitted_generics)]
        ),
        is_classmethod=True,
    )
...
def reparametrize_any_manager_hook(ctx: ClassDefContext) -> None:
    ...
    parent_manager = next(
        (
            base
            for base in manager.node.bases
            if base.type.has_base(fullnames.BASE_MANAGER_CLASS_FULLNAME)
        ),
        None,
    )
