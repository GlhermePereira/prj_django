...
class AddManagers(ModelClassInitializer):
    ...
    def is_manager_dynamically_generated(
        self, manager_info: Optional[TypeInfo]
    ) -> bool:
        if manager_info is None:
            return False
        return (
    ...
    def reparametrize_dynamically_created_manager(
        self, manager_name: str, manager_info: Optional[TypeInfo]
    ) -> None:
...
class ProcessManyToManyFields(ModelClassInitializer):
    ...
    def resolve_many_to_many_arguments(
        ...
        for arg_expr, _arg_kind, arg_name in zip(
            call.args, call.arg_kinds, call.arg_names
        ):
...
class MetaclassAdjustments(ModelClassInitializer):
    ...
    def adjust_model_class(cls, ctx: ClassDefContext) -> None:
        ...
        if (
            does_not_exist is not None
            and isinstance(does_not_exist.node, Var)
            and not does_not_exist.plugin_generated
        ):
        ...
        if (
            objects is not None
            and isinstance(objects.node, Var)
            and not objects.plugin_generated
        ):
    ...
    def add_exception_classes(self) -> None:
        ...
        if "DoesNotExist" not in self.model_classdef.info.names:
            object_does_not_exist = self.lookup_typeinfo_or_incomplete_defn_error(
                fullnames.OBJECT_DOES_NOT_EXIST
            )
            does_not_exist: Union[Var, TypeInfo]
            if self.is_model_abstract:
                does_not_exist = self.create_new_var(
                    "DoesNotExist", TypeType(Instance(object_does_not_exist, []))
                )
                ...
                does_not_exist = helpers.create_type_info(
                    ...
                    self.get_exception_bases("DoesNotExist")
                    or [Instance(object_does_not_exist, [])],
                )
        ...
        if "MultipleObjectsReturned" not in self.model_classdef.info.names:
            django_multiple_objects_returned = (
            ...
            if self.is_model_abstract:
                multiple_objects_returned = self.create_new_var(
                    ...
                    TypeType(Instance(django_multiple_objects_returned, [])),
                )
            ...
            self.model_classdef.info.names[multiple_objects_returned.name] = (
                SymbolTableNode(MDEF, multiple_objects_returned, plugin_generated=True)
            )
