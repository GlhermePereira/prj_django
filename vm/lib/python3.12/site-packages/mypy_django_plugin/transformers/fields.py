...
def set_descriptor_types_for_field(
    ...
    *,
    ...
    if not (
        isinstance(mapped_set_type, UninhabitedType)
        or isinstance(mapped_get_type, UninhabitedType)
    ):
        ...
        if (is_get_nullable or is_nullable) and not (
            isinstance(get_type, NoneType) or helpers.is_optional(get_type)
        ):
            ctx.api.fail(
