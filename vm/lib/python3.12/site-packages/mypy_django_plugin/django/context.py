...
def _get_field_type_from_model_type_info(
...
def _get_field_set_type_from_model_type_info(
    info: Optional[TypeInfo], field_name: str
) -> Optional[MypyType]:
...
def _get_field_get_type_from_model_type_info(
    info: Optional[TypeInfo], field_name: str
) -> Optional[MypyType]:
    field_type = _get_field_type_from_model_type_info(info, field_name)
    if field_type is not None:
...
class DjangoContext:
    ...
    def model_modules(self) -> Dict[str, Dict[str, Type[Model]]]:
        ...
        for concrete_model_cls in self.apps_registry.get_models(
            ...
            modules[concrete_model_cls.__module__][
                concrete_model_cls.__name__
            ] = concrete_model_cls
    ...
    ) -> Iterator["ForeignKey[Any, Any]"]:
        for field in model_cls._meta.get_fields():
            if isinstance(field, ForeignKey):
                yield field

    def get_model_related_fields(
        self, model_cls: Type[Model]
    ) -> Iterator["RelatedField[Any, Any]"]:
    ...
    def get_field_lookup_exact_type(
        ...
        if isinstance(field, (RelatedField, ForeignObjectRel)):
            ...
            primary_key_type = self.get_field_get_type(
                api, rel_model_info, primary_key_field, method="init"
            )
    ...
    def get_field_nullability(
        self, field: Union["Field[Any, Any]", ForeignObjectRel], method: Optional[str]
    ) -> bool:
    ...
    def get_field_get_type(
        ...
        *,
        ...
        if isinstance(field, Field):
            get_type = _get_field_get_type_from_model_type_info(
                model_info, field.attname
            )
        ...
        if isinstance(field, RelatedField):
            ...
            if method in ("values", "values_list"):
                primary_key_field = self.get_primary_key_field(related_model_cls)
                return self.get_field_get_type(
                    api, rel_model_info, primary_key_field, method=method
                )
    ...
    def solve_lookup_type(
        ...
        if (
            lookup == "pk" or lookup.startswith("pk__")
        ) and query.get_meta().pk is None:
